...
class TransportSpec extends org.specs2.mutable.Specification {
  ...
  "process method" should {
    "successfully decode message with valid key and message" in {
      Transport.process("cb67818210934165", message) must beRight
    }
    ...
    "fail to decode message with bad base64 values" in {
      ...
      Transport.process("cb67818210934165", badMessage) must beLeft
    }
    "fail to decode empty message" in {
      Transport.process("cb67818210934165", "") must beLeft
    }

    "fail to decode message with bad key" in {
      Transport.process("adfasdfasdf", message) must beLeft
    }
    "fail to decode message with empty key" in {
      Transport.process("", message) must beLeft
    }
  }
